function[is_outlier] = detect_outliers(X,mu,C)
[Nbp,Ndim] = size(X);
...
while(~stop_search)
    ...
    for n = 1:Nlist
        ...
        indCsub = [list_bp(n,:) list_bp(n,:)+Nbp];
        %indCsub = [list_bp(n,:) list_bp(n,:)+Nbp list_bp(n,:)+2*Nbp];%3d 
   ...
   Coef_th=25;
    TH = Coef_th*chi2inv(0.99,Ndim*Ninlier);
    if minC<TH
        stop_search = true;
